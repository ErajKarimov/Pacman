#ifndef HEADERS_GHOST_HPP
#define HEADERS_GHOST_HPP
...
public:
    explicit Ghost(unsigned char i_id);

    [[nodiscard]] bool pacman_collision(const Position &i_pacman_position) const;

    [[nodiscard]] float get_target_distance(unsigned char i_direction) const;
