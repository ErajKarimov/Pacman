...
float Ghost::get_target_distance(unsigned char i_direction) const {
    short x = position.x;
    short y = position.y;
...
void Ghost::draw(bool i_flash, sf::RenderWindow &i_window) {
    ...
    auto body_frame = static_cast<unsigned char>(std::floor(animation_timer / static_cast<float>(GHOST_ANIMATION_SPEED)));
...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH> &i_map, Ghost &i_ghost_0, Pacman &i_pacman) {
    ...
    walls[0] = map_collision(false, use_door, speed + position.x, position.y, i_map);
    walls[1] = map_collision(false, use_door, position.x, position.y - speed, i_map);
    walls[2] = map_collision(false, use_door, position.x - speed, position.y, i_map);
    walls[3] = map_collision(false, use_door, position.x, speed + position.y, i_map);
